...
export default function VenueOwnerPanel({ view = 'overview' }: { view?: 'overview' | 'bookings' | 'availability' | 'stats' }) {
  ...
  async function deleteBlock(blockId: string) {
    try { await removeBlock({ blockId: blockId as never }) } catch (caught) { setBlockError(caught instanceof Error ? caught.message : 'Could not remove maintenance block') }
  }
