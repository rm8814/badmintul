...
export default function AppShell({ role, children }: { role: Role; children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-brand-bg"><header className="border-b border-neutral-200/70 bg-white/90 px-5 py-4 shadow-sm backdrop-blur-md sm:px-8"><div className="content-container flex flex-wrap items-center justify-between gap-4"><a className="shrink-0 text-xl font-extrabold tracking-tight text-neutral-900" href={roleRoutes[role]}>badmintul<span className="text-brand-primary">.</span></a><div className="flex flex-wrap items-center gap-3"><span className="rounded-full bg-brand-primary/10 px-3 py-1 text-xs font-bold text-brand-primary">{roleLabels[role]}</span><Button variant="secondary" disabled={isSigningOut} onClick={() => void signOutAndReturnHome()}>{isSigningOut ? 'Signing out…' : 'Sign out'}</Button></div></div></header>{children}</div>
}
