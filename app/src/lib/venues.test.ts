...
describe('Task 5 venue ownership', () => {
  it('requires at least one valid court server-side', async () => {
    ...
    await expect(owner.mutation(api.venues.createVenueWithCourts, { name: 'Test venue', address: 'Address', description: '', photos: [], city: 'Jakarta', courts: [] })).rejects.toThrow('At least one court is required')
  })
  ...
  it('rejects an owner attempting to read another owner venue', async () => {
    ...
    const venueB = await t.withIdentity({ subject: ownerB }).mutation(api.venues.createVenueWithCourts, { name: 'B venue', address: 'B address', description: '', photos: [], city: 'Jakarta', courts: [{ name: 'B court', pricePerHour: 100, operatingHours: { open: '08:00', close: '22:00' } }] })
